...
def check_port(argv1, argv2, argv3):
    """
        This function checks the user input if meet the requirement:
        Port number should between 1024 and 64000,
        Also it has to be able to transformed as a int so if contains any invalid characters
...
def main(ports):
    """This is the main server function, initialize the whole server terminal."""
    ...
    data_received = [sock_1, sock_2, sock_3]

    rd, wd, ex = select.select(data_received, [], [])
    while 1:

        for received_socket in rd:

            response_data = bytearray()  # Initialize the response data which will be sent to client terminal
            time_or_date = ''
            ...
            """Add year, month, day, hour, minute to DT_response byte array"""
            ...
            """
               Select the language type, generate requested text type
               and add to DT_response byte array
            """
